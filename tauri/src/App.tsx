...
export default function UnlovableLanding() {
  ...
  const [tempProvider, setTempProvider] = useState("Ollama");
  ...
  useEffect(() => {
    // Fire-and-forget quit signal using sendBeacon (works even after window is gone)
    const cleanup = () => {
      if (projectOpenSuccess) return;

      // This will send the POST even if the app is already closing
      navigator.sendBeacon("http://localhost:8000/api/quit");
    };

    // Catch ALL close attempts: X button, Alt+F4, Cmd+Q, taskbar close, etc.
    ...
    const unlisten = getCurrentWindow().onCloseRequested((event) => {
      ...
      // Don't await anything. Just tell Rust we're quitting.
      fetch("http://localhost:8000/api/quit", { method: "POST" }).catch(
        () => {},
      );
      // Do NOT call event.preventDefault()
      // Do NOT await
      // Do NOT return a promise that never resolves
    });

    // Cleanup listeners on unmount (safe because we're not awaiting anything)
    return () => {
      window.removeEventListener("beforeunload", cleanup);
      unlisten.then((f) => f?.()).catch(() => {});
    };
  }, [projectOpenSuccess]); // Re-run if project state changes
      ...
      {isLoading ? (
        ...
        <div className="min-h-screen relative overflow-hidden bg-slate-950">
          ...
          <div className="relative z-10">
            ...
            <div className="max-w-6xl mx-auto px-6 pt-20 pb-18 text-center">
              <motion.div
              ...
              >
                No frameworks. No build steps. No complexity. Just describe what
                you want, and unlovable creates beautiful, functional
