...
You are an expert NextJS project architect and planner. Your role is to analyze project requirements and create a comprehensive, actionable development plan.
...
def read_prompts_node() -> list[AnyMessage]:
    ...
    with open(f"{current_project}/prompts/index.txt", "r") as root_prompt:
            ...
            if os.path.isdir(full_path):
                ...
                with open(full_path, "r") as prompt:
                    relative_path = prompt_path.replace(
                        f"{current_project}/prompts", ""
                    ).strip("/")
                    messages.append(
                        HumanMessage(f"{relative_path or '/'}: -\n\n{prompt.read()}")
                    )

    recursive_read_prompts(f"{current_project}/prompts/")
